...
var mysqlQ = require('./mysql');
log.info('==mysql.push:== %j)', mysqlQ.push);
log.info('==mysql.initializeDatabase:== %j)', mysqlQ.initializeDatabase);
...
function mysql() {
  return new Promise(function(resolve, reject) {
    ...
    function setupConsumer() {
      ...
      // Consumer
      log.info('==mysql.initializeDatabase:== %j)', mysqlQ.initializeDatabase);
      log.info('==mysql.push:== %j)', mysqlQ.initializeDatabase);
      if (true /*jobData.done*/ ) {
        // log.info('This would be a good time to resolve %d',rabbitSum);
    ...
    // Publisher
    log.info('would publish %d', maxCount);
    ...
    source.map(function(x) {
      ...
      .subscribe(function(result) {
          log.info('async return for id:%d', result.insertId);
        }, function(error) {
          log.error('onError: %s', error);
        },
        function() {
